...
def main():
    ...
    tmpdir = "%s/elastos_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    def get_next(self):
        while self.num_running < self.num_jobs and self.test_list:
            ...
            self.jobs.append((t,
                              time.time(),
                              subprocess.Popen(
                                  [self.tests_dir + test_argv[0]] + test_argv[1:] + self.flags + portseed_arg + tmpdir,
